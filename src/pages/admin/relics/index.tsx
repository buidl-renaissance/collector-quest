...
import { FaArrowLeft, FaPlus, FaEdit, FaTrash, FaCrown, FaSearch, FaGem } from 'react-icons/fa';
...
const AdminPage: React.FC<AdminPageProps> = ({ relics: initialRelics }) => {
  ...
  const handleCreateRelic = () => {
    router.push('/relics/create');
  };

  const handleEditRelic = (id: string) => {
    router.push(`/relics/${id}/edit`);
  };

  const handleDeleteRelic = async (id: string) => {
    if (!confirm('Are you sure you want to delete this relic? This action cannot be undone.')) {
      return;
    }
    ...
    try {
      const response = await fetch(`/api/relics/${id}`, {
        method: 'DELETE',
      });

      if (!response.ok) {
        throw new Error('Failed to delete relic');
      }

      setRelics(relics.filter(relic => relic.id !== id));
    } catch (err) {
      console.error('Error deleting relic:', err);
      setError('Failed to delete relic. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredRelics = relics.filter(relic => 
    ...
    (relic.story?.toLowerCase() || '').includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <Container>
      ...
      <Title>Relic Administration</Title>
      <Subtitle>Manage powerful relics and their associated artifacts</Subtitle>
      ...
      <ActionButton onClick={handleCreateRelic}>
        <FaPlus /> Create New Relic
      </ActionButton>

      <RelicsList>
        {filteredRelics.length === 0 ? (
          <EmptyState>No relics found. Create your first relic!</EmptyState>
        ) : (
          filteredRelics.map(relic => (
            <RelicItem key={relic.id}>
              ...
              <RelicActions>
                <ActionButton onClick={() => handleEditRelic(relic.id)}>
                  <FaEdit /> Edit
                </ActionButton>
                <DeleteButton onClick={() => handleDeleteRelic(relic.id)}>
                  <FaTrash /> Delete
                </DeleteButton>
              </RelicActions>
            </RelicItem>
...
const RelicsList = styled.div`
  display: flex;
  flex-direction: column;
  gap: 1.5rem;
`;

const RelicItem = styled.div`
  background: rgba(26, 26, 46, 0.8);
  border: 1px solid #bb8930;
  border-radius: 8px;
  padding: 1.5rem;
...
const RelicDescription = styled.p`
  color: #e0dde5;
  margin-bottom: 1rem;
  font-size: 1rem;
  line-height: 1.5;
