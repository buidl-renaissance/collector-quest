...
import { FaArrowLeft, FaPlus, FaEdit, FaTrash, FaCrown, FaSearch } from 'react-icons/fa';
...
const AdminPage: React.FC<AdminPageProps> = ({ artifacts: initialArtifacts }) => {
  ...
  const handleEditArtifact = (id: string) => {
  ...
  return (
    <Container>
      ...
      <Title>Artifact Administration</Title>
      <Subtitle>Manage artifacts and their associated relics</Subtitle>
      ...
      <ActionButton onClick={handleCreateArtifact}>
        <FaPlus /> Create New Artifact
      </ActionButton>

      <ArtifactsList>
        {filteredArtifacts.length === 0 ? (
          <EmptyState>No artifacts found. Create your first artifact!</EmptyState>
        ) : (
          filteredArtifacts.map(artifact => (
            <ArtifactItem key={artifact.id}>
              <ArtifactInfo>
                ...
                <ArtifactDetails>
                  ...
                  <ArtifactDescription>{artifact.description.substring(0, 150)}...</ArtifactDescription>
                  {artifact.relic && (
                    <RelicBadge>
                      <CrownIcon><FaCrown /></CrownIcon>
                      Relic: {artifact.relic.name}
                    </RelicBadge>
                  )}
                </ArtifactDetails>
              </ArtifactInfo>
              <ArtifactActions>
                <ActionButton onClick={() => handleEditArtifact(artifact.id)}>
                  <FaEdit /> Edit
                </ActionButton>
                <DeleteButton onClick={() => handleDeleteArtifact(artifact.id)}>
                  <FaTrash /> Delete
                </DeleteButton>
              </ArtifactActions>
            </ArtifactItem>
