...
import { FaArrowLeft, FaScroll, FaDice, FaFeather, FaSpinner } from "react-icons/fa";
...
const CharacterPage: React.FC = () => {
  ...
  useEffect(() => {
    // Check if we have a character loaded
    if (character) {
      setLoading(false);
    } else {
      // If no character is loaded, check if we have a character ID in storage
      const characterId = getCurrentCharacterId();
      if (characterId) {
        // If we have an ID but no character, redirect to the character page with ID
        router.push(`/character/${characterId}`);
      } else {
        // If no character ID, redirect to home
        router.push("/");
      }
    }
  }, [character, router]);
