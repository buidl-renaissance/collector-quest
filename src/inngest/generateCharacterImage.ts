    ...
    try {
      ...
      const generatedImage = await step.run("generate-image", async () => {
        const imagePrompt = prompt || 
          `character portrait based on facial analysis: ${facialData} and character sex: ${character.sex}, race: ${character.race?.name} and class: ${character.class?.name}, change character from holding the current item to holding something specific to their race/class`;
